...
### Breaking Changes
* API method parameters declared with a `null` default are now validated against their declared type. Such a parameter that is supplied with a value its declared type cannot accept, for example an array where a string is expected, is rejected with a bad request error instead of silently falling back to `null`. Previously only parameters without a default value were validated this way; parameters declared with any other default still fall back to it. A parameter that is not supplied, or is supplied without a value (`&param=`), continues to use its default. This applies to API classes that set `$autoSanitizeInputParams = false` or methods annotated `@unsanitized`; every other API method resolves its parameters through the sanitizing path and is unaffected.
