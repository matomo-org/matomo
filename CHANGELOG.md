...
### Breaking Changes
* API method parameters are now validated against their declared type. A parameter that is supplied with a value its declared type cannot accept, such as an array where a string is expected, is rejected with a bad request error instead of silently falling back to the parameter's default value. Previously this validation applied only to parameters that have no default. Parameters that are not supplied at all continue to use their default as before.
