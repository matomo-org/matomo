...
* `AjaxHelper` gained a `rejectOnAbort` option. An aborted request normally settles the promise returned by `AjaxHelper.fetch()` neither way, because navigating the page aborts every queued request at once. A caller that outlives the abort, such as an auto-refresh loop, can pass `rejectOnAbort: true` to have it rejected with the jqXHR instead.
* The new `isRetryableRequestError()` helper in CoreHome reports whether a failed request is worth sending again. It returns `true` only for a request that never completed (aborted, dropped, timed out) or that the server could not answer (5xx, 429); a rejected authentication, an unparsable response and anything unrecognised return `false`, so a caller polling in a loop stops rather than repeating a request the server already refused.
