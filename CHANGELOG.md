...
### Breaking Changes
* The interface `Piwik\Settings\Interfaces\PolicyComparisonInterface` gained five methods used by the granular compliance dashboard: `getPolicySettingId()`, `isExternallyManagedByPolicyPage()`, `getWhatItDoes(?int $idSite = null)`, `getImpact(?int $idSite = null)` and `getComplianceTitle(?int $idSite = null)`. Plugins that implement the interface directly must implement them. Plugins using `Piwik\Settings\Interfaces\Traits\PolicyComparisonTrait` (as all known implementers do) inherit default implementations and are not affected, except for subclasses of `Piwik\Plugins\PrivacyManager\Settings\CompliancePolicyEnforcedSetting` — see below.
* `Piwik\Plugins\PrivacyManager\Settings\CompliancePolicyEnforcedSetting` now declares `getWhatItDoes(?int $idSite = null)` abstract in place of `getComplianceRequirementNote()`. A class-level abstract declaration takes precedence over the trait's default, so subclasses that only implemented `getComplianceRequirementNote()` now fail to load and must implement `getWhatItDoes()` instead. Overriding `getImpact(?int $idSite = null)` is optional but recommended, since the compliance dashboard renders it as its own column.
* `Piwik\Settings\Interfaces\Traits\PolicyComparisonTrait` now declares `getTitle()` abstract, because its `getComplianceTitle()` default returns it. Every setting reachable from the compliance dashboard already implements `getTitle()` via `Piwik\Settings\Interfaces\SettingValueInterface`, so this only surfaces a missing implementation at class-load time instead of fataling when the dashboard renders.
