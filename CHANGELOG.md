...
* `API.getReportMetadata` no longer returns the goal overview reports that carried `idGoal=0`: `Goals_get_idGoal--0`, `Goals_getVisitsUntilConversion_idGoal--0` and `Goals_getDaysToConversion_idGoal--0`. None of them held the goals overview figures their name promised: `Goals.get` reads `idGoal=0` as the ecommerce order goal, while `Goals.getVisitsUntilConversion` and `Goals.getDaysToConversion` read it as all goals. Stored scheduled reports and alerts, as well as `ScheduledReports.addReport` and `ScheduledReports.updateReport` requests, that still use one of these unique ids resolve it to the report holding the same figures: `Goals_get_idGoal--0` to `Goals_get_idGoal--ecommerceOrder` on sites with ecommerce enabled, and `Goals_getVisitsUntilConversion_idGoal--0` and `Goals_getDaysToConversion_idGoal--0` to `Goals_getVisitsUntilConversion` and `Goals_getDaysToConversion` on every site. On a site without ecommerce `Goals_get_idGoal--0` resolves to `Goals_get`, so such a scheduled report or alert shows the sum across all goals instead of the ecommerce order figures, which are always zero there. Report metadata looked up for one of these reports with `idGoal=0`, as by `API.getProcessedReport` and `API.getRowEvolution`, resolves the same way: for `Goals.get` to `Goals_get_idGoal--ecommerceOrder`, which keeps returning the ecommerce order figures and on a site without ecommerce now fails with report not found, and for `Goals.getVisitsUntilConversion` and `Goals.getDaysToConversion` to their all-goals report on every site. The protected `Piwik\Plugins\Goals\Reports\Base::addReportMetadataForEachGoal()` no longer adds these `idGoal=0` reports and its `$isGoalSummaryReport` parameter has been removed; a subclass that still passes it keeps working but no longer gets the overview report. The translation key `Goals_GoalsOverview` has been removed.
