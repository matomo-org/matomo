...
* The new `Piwik\Scheduler\Scheduler::rescheduleTaskAndRunNow()` marks a scheduled task as due immediately, so the next scheduler run executes it and then returns it to its own schedule. It joins the existing `rescheduleTaskAndRunTomorrow()` and `rescheduleTaskAndRunInOneHour()`, and suits a task a plugin wants brought forward after an event such as an installation or an update. The timetable holds one time per task, so calling it repeatedly before that run still results in a single execution.
