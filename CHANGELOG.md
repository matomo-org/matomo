...
* An enriched plugin's `consumer.license` is now the license `Piwik\Plugins\Marketplace\Consumer::getConsumerPluginLicenses()` reports rather than the copy the Marketplace embedded in the plugin list, so it agrees with the `hasExceededLicense` and `isMissingLicense` flags beside it. It is replaced only for an installed plugin, which is the only kind whose license state is resolved at all. This also changes what `Marketplace.searchPlugins` sends to the browser, since `consumer` is one of the fields the plugin cards keep. `Piwik\Plugins\Marketplace\Plugins\InvalidLicenses` classifies the admin-page license banners from this value, so those banners and the plugin cards can no longer disagree; `createAccount` and `startFreeTrial` now clear its cache alongside the Marketplace cache, as `saveLicenseKey` already did.
