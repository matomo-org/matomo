...
* The plugin code generators have been removed: `generate:api`, `generate:archiver`, `generate:command`, `generate:controller`, `generate:dimension`, `generate:menu`, `generate:plugin` (and its `generate:theme` alias), `generate:report`, `generate:scheduledtask`, `generate:settings`, `generate:system-check`, `generate:test`, `generate:update`, `generate:visualizationplugin` and `generate:widget`. They copied a file out of one of the `Example*` plugins and renamed a few identifiers; use the matching example plugin as the starting point instead. `Piwik\Plugins\CoreConsole\Commands\GeneratePluginBase` is kept, as the `generate:tagmanager-*` and `generate:vue-component` commands build on it.
