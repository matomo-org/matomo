...
* A plugin that the Marketplace's update check reports as updatable is now marked updatable when the Marketplace plugin enriches it, without separately resolving that plugin's full info first. `Piwik\Plugins\Marketplace\Plugins::getPluginsHavingUpdate()` keeps its signature and its filtering, but `Api\Client::getInfoOfPluginsHavingUpdate()` now answers from the cached plugin and theme catalogues, falling back to an info request — and to dropping the plugin when that comes back empty — only for a plugin neither catalogue lists. A custom or delisted plugin is filtered out exactly as before. Against plugins.matomo.org the resulting set was identical, but an installation where the catalogues and the info endpoint disagree will now see any reported update the catalogues can describe.
...
* Data the Marketplace itself authors — the description, readme and FAQ, screenshots, authors, activity and version list, plus the `priceFrom` and `missingRequirements` computed from them — is now held for ninety minutes rather than one hour, and only for the unfiltered, default-sorted lists the overview asks for. A search, a different sort or a purchase type nothing warms keeps the one hour timeout it had. The new hourly `Marketplace.warmCacheEntries` task refills those lists on the hour, so the extra thirty minutes is headroom for a missed cron run rather than staleness a page normally shows. Anything derived from local state or a separately cached response is unchanged: `isInstalled`, `isActivated`, `canBeUpdated`, and everything about the consumer's license. The whole Marketplace cache is still cleared by the "check for updates" action, and by saving a license key, starting a trial, creating an account, opening the subscription overview or applying a core update.
