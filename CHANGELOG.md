...
* Installations that were created before `archive_blob_*` tables used a `LONGBLOB` value column can still have `MEDIUMBLOB` ones, which silently truncate a compressed report blob at 16 MB. On such an installation the Matomo 6 update sets the new `[database] archive_blob_tables_may_contain_mediumblob` config setting, and while it is set any configured `datatable_archiving_maximum_rows_*` limit above `100000` — most relevant for `datatable_archiving_maximum_rows_actions_flat` — is reduced to `100000` when archiving into a table that is still `MEDIUMBLOB`. Tables created since the schema change are unaffected, so this only applies when re-archiving periods that predate the upgrade, and the reduction is logged once per table. Run `./console core:recheck-archive-blob-types` to list the affected tables and get the `ALTER TABLE` statements; once they are all converted the command removes the setting and the configured limits apply again.
