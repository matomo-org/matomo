...
* The `Marketplace.searchPlugins` controller action now returns only the fields the plugin cards in the Marketplace list render. The version history, shop details, screenshots, support links, authors, changelog and activity it used to include are served per plugin by the new `Marketplace.getPluginDetails` action instead, which a plugin's details page requests when it opens. Each version of every plugin carried its own rendered readme and FAQ HTML, so the list response was over a megabyte on a typical install. The `plugins` template variable the `Marketplace.GetPremiumFeatures` and `Marketplace.GetNewPlugins` widgets pass to their templates has been reduced the same way.
...
* The new `Piwik\Plugins\Marketplace\Plugins::getPluginInfoPreferringList()` returns one plugin's full information, answering from an already cached overview list when it holds that plugin and falling back to `Api\Client::getPluginInfo()` otherwise. `Marketplace.getPluginDetails` uses it, so opening a plugin's details page on a warm cache costs no Marketplace request. The supporting `Api\Client::findInCachedOverviewLists()` searches only the lists that are already cached, so a cold cache never turns a single plugin lookup into a download of a whole catalogue.
