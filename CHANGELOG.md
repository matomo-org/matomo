...
* `Piwik\Category\CategoryList::addCategory()` and `Piwik\Category\Category::addSubcategory()` now throw an exception when the given category or subcategory has no id, instead of silently registering it under the empty-string key. A plugin that declares a `Category` or `Subcategory` component without an `$id`, or that pushes one onto the `Category.addSubcategories` event, must set an id.
