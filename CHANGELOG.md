...
* The new `Piwik\Plugins\Marketplace\Plugins::getPluginInfoPreferringList()` returns one plugin's full information, answering from an already cached overview list when it holds that plugin and falling back to `Api\Client::getPluginInfo()` otherwise. `Marketplace.getPluginDetails` uses it, so opening a details modal on a warm cache costs no Marketplace request. The supporting `Api\Client::getCachedOverviewList()` returns an overview list only when it is already cached, and `null` when it is not, so a cold cache never turns a single plugin lookup into a download of the whole catalogue.
