...
class ArchiveTableCreator
{
     ...
     * but this is deprecated and will change in Matomo 7. Pass `true` to explicitly create missing tables or
    ...
    /**
     * @deprecated Will be removed in Matomo 7 once omitted get*Table() calls stop using the legacy default behavior.
     */
    private static function triggerLegacyDefaultDeprecation(string $type): void
    {
        ...
        trigger_error(
            sprintf(
                "Omitting \$createIfMissing in ArchiveTableCreator::%s() is deprecated. Pass true to create missing archive tables or false to only return existing ones. In Matomo 7 the default behavior will change to lookup-only.",
                $method
            ),
            E_USER_DEPRECATED
        );
