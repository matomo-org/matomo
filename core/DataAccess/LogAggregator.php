...
class LogAggregator
{
    ...
    public function generateQuery($select, $from, $where, $groupBy, $orderBy, $limit = 0, $offset = 0)
    {
        ...
        if (!$this->segment->isEmpty() && $this->isSegmentCacheEnabled()) {
            ...
            Db::getReader()->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . Common::prefixTable($segmentTable) . ' (idvisit  BIGINT(10) UNSIGNED NOT NULL) ENGINE=MEMORY ' . $segmentSql['sql'], $segmentSql['bind']);
