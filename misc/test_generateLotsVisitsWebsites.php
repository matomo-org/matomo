...
class Piwik_StressTests_CopyLogs
{
	...
	function run()
	{
		...
		WHERE date(visit_last_action_time) between '$startDate' and '$endDate' ;";
		$result = $db->query($sql);
	...
	{
		$sql = "SELECT count(*) FROM `". Piwik_Common::prefixTable('log_visit')."` WHERE DATE(`visit_last_action_time`) = CURRENT_DATE;";
		return Zend_Registry::get('db')->fetchOne($sql);
	}
	function getConversionItemsToday($table = 'log_conversion_item')
	{
		$sql = "SELECT count(*) FROM `".Piwik_Common::prefixTable($table)."` WHERE DATE(`server_time`) = CURRENT_DATE;";
		return Zend_Registry::get('db')->fetchOne($sql);
	}
	...
	{
		$sql = "SELECT count(*) FROM `". Piwik_Common::prefixTable('log_link_visit_action')."` WHERE DATE(`server_time`) = CURRENT_DATE;";
		return Zend_Registry::get('db')->fetchOne($sql);
	}
}
