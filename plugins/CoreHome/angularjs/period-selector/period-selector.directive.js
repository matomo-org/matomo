...
(function () {
    ...
    function piwikPeriodSelector(piwik, $location) {
        return {
            restrict: 'A',
            scope: {
                // TODO: pretty date should be set by date value in function
                prettyDate: '@',
                prettyDateLong: '@',
                periods: '<',
                periodTranslations: '<'
            ...
            link: function (scope, element, attrs) {
                scope.periodSelector.selectedPeriod = piwik.period;

                // TODO: refactory (copied from calendar.js)
                function getWeek(date) {
                    var onejan = new Date(date.getFullYear(), 0, 1), // needed for getDay()
                ...
                function isDateInCurrentPeriod(date) {
                    ...
                    if (scope.periodSelector.selectedPeriod != piwik.period) {
                        return [true, ''];
                    }
                ...
                function getDatePickerOptions() {
                    ...
                    result.stepMonths = scope.periodSelector.selectedPeriod == 'year' ? 12 : 1;
                ...
                var highlightCurrentPeriod = function () {
                    switch (scope.periodSelector.selectedPeriod) {
                        case 'day':
                            // highlight this link
                ...
                var unhighlightAllDates = function () {
                    ...
                    if (piwik.period == 'year') {
                        var viewedYear = $('.ui-datepicker-year', datepickerElem).val(),
                            toggle = scope.periodSelector.selectedPeriod == 'year' && currentYear == viewedYear;
                ...
                updateDate = function (dateText) {
                    ...
                    updateDisplayDate(scope.periodSelector.selectedPeriod, dateText);
                    ...
                    propagateNewUrlParams(dateText, scope.periodSelector.selectedPeriod);
                    initTopControls();
                    reloading = false;
                };
                ...
                var toggleMonthDropdown = function (disable) {
                    if (typeof disable === 'undefined') {
                        disable = scope.periodSelector.selectedPeriod == 'year';
                    }
                ...
                datepickerElem.on('mouseenter', 'tbody td', function () {
                    ...
                    if ($(this).hasClass('ui-state-disabled') && scope.periodSelector.selectedPeriod != 'year') {
                        unhighlightAllDates();

                        // if period is week, then highlight the current week
                        if (scope.periodSelector.selectedPeriod == 'week') {
                ...
                var changePeriodWithPageReload = function (periodInput) {
                    ...
                    var id = periodInput.attr('id'),
                        period = id.substring('period_id_'.length);
                ...
                var changePeriodOnClickIfPeriodChanged = function (periodInput) {
                    ...
                    // if clicking on the selected period, change the period but not the date
                    if (scope.periodSelector.selectedPeriod == period && scope.periodSelector.selectedPeriod != 'range') {
                        // only reload if current period is different from selected
                        if (piwik.period != scope.periodSelector.selectedPeriod) {
                ...
                $("#otherPeriods").on('dblclick', 'label,input', function (e) {
                ...
                $("#otherPeriods").on('click', 'input', function (e) {
                    var id = $(e.target).attr('id'),
                        period = id.substring('period_id_'.length),
                        lastPeriod = scope.periodSelector.selectedPeriod;
                    ...
                    if (scope.periodSelector.selectedPeriod == 'year' || lastPeriod == 'year') {
                        ...
                        datepickerElem
                            .datepicker('option', 'stepMonths', scope.periodSelector.selectedPeriod == 'year' ? 12 : 1)
                ...
                $(datepickerElem).on('click', '.ui-datepicker-next,.ui-datepicker-prev', function () {
                    ...
                    toggleMonthDropdown(scope.periodSelector.selectedPeriod == 'year');
                });
