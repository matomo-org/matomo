...
export default defineComponent({
  ...
  setup(props) {
    // getSortedSparklines() groups entries by key; flatten back into display order.
    const flatSparklines = computed<SparklineEntry[]>(
      () => ([] as SparklineEntry[]).concat(...Object.values(props.sparklines || {})),
    );
    ...
    onMounted(() => {
      ...
      // unbinds before binding, so re-invoking it is safe.
      nextTick(() => {
        if (typeof window.initializeSparklines === 'function') {
