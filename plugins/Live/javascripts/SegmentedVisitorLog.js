var SegmentedVisitorLog = function() {
    ...
    function show(apiMethod, segment, extraParams) {
        ...
        var callback = function (html) {
            ...
            // remove title returned from the server. A report renders its heading inside the
            // shared ReportHeader component, so drop that whole host element rather than just
            // the heading, which would leave the header shell behind.
            var title = box.find('.enrichedHeadline')
                .closest('[vue-entry="CoreHome.ReportHeader"], h2');
