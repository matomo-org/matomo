...
class VisitsWithPagePerformanceMetrics extends Fixture
{
    ...
    protected function trackVisits()
    {
        ...
        $t = self::getTracker($this->idSite, $dateTime, $defaultInit = true);
        $t->setUrl('http://example.org/category/Page1');
        $t->setPerformanceTimings(355, 444, 1300, 512, 333);
        self::checkResponse($t->doTrackPageView('Page Title 1'));
        $t->setUrl('http://example.org/Contact');
        $t->setPerformanceTimings(111, 278, 988, 355, 66);
        self::checkResponse($t->doTrackPageView('Contact'));
        $t->setUrl('http://example.org/Contact/ThankYou');
        $t->setPerformanceTimings(211, 399, 998, 355, 222);
        self::checkResponse($t->doTrackPageView('Contact'));
        ...
        $t = self::getTracker($this->idSite, $dateTime, $defaultInit = true);
        $t->setUrl('http://example.org/category/Page1');
        $t->setPerformanceTimings(277, 388, 1025, 436, 299);
        self::checkResponse($t->doTrackPageView('Page Title 1'));
        $t->setUrl('http://example.org/Contact');
        $t->setPerformanceTimings(99, 199, 899, 236, 100);
        self::checkResponse($t->doTrackPageView('Contact'));
        $t->setUrl('http://example.org/Contact/ThankYou');
        $t->setPerformanceTimings(123, 255, 1200, 233, 258);
        self::checkResponse($t->doTrackPageView('Contact'));
        ...
        $t = self::getTracker($this->idSite, $dateTime, $defaultInit = true);
        $t->setUrl('http://example.org/category/Page1');
        $t->setPerformanceTimings(158, 136, 1235, 359, 248);
        self::checkResponse($t->doTrackPageView('Page Title 1'));
        $t->setUrl('http://example.org/Contact');
        $t->setPerformanceTimings(132, 205, 1125, 236, 135);
        self::checkResponse($t->doTrackPageView('Contact'));
        $t->setUrl('http://example.org/Contact/ThankYou');
