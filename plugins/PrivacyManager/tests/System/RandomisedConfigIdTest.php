...
class RandomisedConfigIdTest extends SystemTestCase
{
    ...
    public function testNormalConfigIdBehaviour()
    {
        ...
        $count = Db::fetchAll('SELECT idvisitor, COUNT(1) as conversions FROM ' . Common::prefixTable('log_conversion') . ' WHERE DATE(server_time) = DATE("' . self::$dateTimeNormalConfig . '") GROUP BY idvisitor');
    ...
    public function testConfigIdRandomised()
    {
        ...
        $count = Db::fetchAll('SELECT idvisitor, COUNT(1) as conversions FROM ' . Common::prefixTable('log_conversion') . ' WHERE DATE(server_time) = DATE("' . self::$dateTimeRandomisedConfig . '") GROUP BY idvisitor');
