...
import { MatomoUrl, SiteSelector } from 'CoreHome';
...
function scopeField(wrapper: ReturnType<typeof mount>) {
  return wrapper.findComponent(Field);
}
...
describe('PrivacyManager/Compliance', () => {
  it('configures every website when no scope is requested', async () => {
    ...
    expect(wrapper.findComponent(SiteSelector).exists()).toBe(false);
  ...
  it('configures the requested single website', async () => {
    ...
    expect(wrapper.findComponent(SiteSelector).exists()).toBe(true);
  ...
  it('waits for a website to be picked before configuring a single website', async () => {
    ...
    await wrapper.findComponent(SiteSelector).vm.$emit('update:modelValue', { id: 3, name: 'Site 3' });
    await flushPromises();
  ...
  it('never offers All Websites inside the website selector', async () => {
    ...
    expect(wrapper.findComponent(SiteSelector).props('showAllSitesItem')).toBe(false);
  });
