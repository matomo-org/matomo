...
<template>
  ...
  >
    ...
    >
      <template v-if="enabled && !enableDeleteReports">
        <h2>{{ translate('PrivacyManager_DeleteLogsConfirmTitle') }}</h2>
        <p>{{ translate('PrivacyManager_DeleteLogsConfirmBody') }}</p>
      </template>
      <template v-if="enabled && enableDeleteReports">
...
export interface DeleteOldLogsState {
  isLoading: boolean;
  enabled: boolean;
