...
describe("SegmentSelectorEditorTest", function () {
    ...
    async function resetComparedSegments() {
      await page.waitForFunction(() => !!window.CoreHome && !!window.CoreHome.ComparisonsStoreInstance);

      await page.evaluate(() => {
        const comparisonsStore = window.CoreHome && window.CoreHome.ComparisonsStoreInstance;
        if (!comparisonsStore) {
          return;
        }

        const compared = comparisonsStore.getSegmentComparisons();
        for (let i = compared.length - 1; i > 0; i -= 1) {
          comparisonsStore.removeSegmentComparison(i);
        }
      });
    }
