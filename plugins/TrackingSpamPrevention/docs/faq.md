...
For this to work a geolocation provider must be enabled, and "Block tracking requests from Cloud hosting providers" in "Administration => General Settings" must be set to "Use custom organisation list". Selecting that option pre-fills the "Organisation block list" with Matomo's default set of hosting and datacenter providers, which you can then extend or trim as needed.
