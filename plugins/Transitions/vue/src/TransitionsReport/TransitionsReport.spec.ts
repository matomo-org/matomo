...
// Required inside the factory: jest hoists it above every import in the file, so it cannot reach
// a top-level one. postEvent is only dereferenced when the closure runs, well after it is assigned.
jest.mock('CoreHome', () => {
  // eslint-disable-next-line global-require
  const { coreHomeMock } = require('./testCoreHomeMock');
...
describe('Transitions/TransitionsReport', () => {
  ...
  beforeEach(() => {
    postEvent.mockClear();
    stubElementRects();
    backend = installFakeTransitionsBackend(JSON.parse(JSON.stringify(REPORT_WITH_DATA)));
    useSynchronousFrames();
  });
