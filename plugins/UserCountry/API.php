...
class API extends \Piwik\Plugin\API
{
    ...
    public function getRegion($idSite, $period, $date, $segment = false)
    {
        ...
        // convert fips region codes to iso
        // first we check if the start date of the period is before the date we switched to ISO
        // if this is the case we do not need to convert any codes at all
        // if not we try to convert the codes to ISO, if the code is mappable
        // Note: as all old codes are mapped, not mappable codes need to be iso codes already, so we leave them
        if (!$this->areRegionCodesIso($date, $period)) {
            $dataTables = [$dataTable];
            ...
            foreach ($dataTables as $dt) {
                $dt->filter('GroupBy', array(
                    ...
                    function ($label) use ($separator, $unk) {
                        ...
                        //
                        list($countryCode, $regionCode) = LocationProvider\GeoIp::convertRegionCodeToIso($countryCode,
                            $regionCode, true);
    ...
    private function areRegionCodesIso($date, $period)
    {
