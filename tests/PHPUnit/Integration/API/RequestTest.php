...
class RequestTest extends IntegrationTestCase
{
    ...
    public function testReloadAuthRestoresSuperUserPermissionWhenTokenAuthenticatesNothing()
    {
        ...
        // A caller that stringifies an absent token sends a non-empty value that is no credential — the
        // `'' + params.token_auth` idiom yielding "undefined", which two of Matomo's own UserCountryMap
        // requests used. The reset before authentication still has to happen so a scoped token cannot
        // inherit ambient access, but once nothing authenticated there is no scope to enforce and the
        // access the request arrived with has to come back.
