...
class AccessTest extends IntegrationTestCase
{
    ...
    public function testCheckUserHasCapabilityLoadsSitesWhenNothingElseHasLoadedThemYet()
    {
        Piwik::addAction('Access.Capability.addCapabilities', function (&$cap) {
            $cap[] = new TestCustomCap();
        });
        ...
        // Capability site ids are derived from the loaded roles, so a capability check that is the first
        // access read of a request has to load them itself. Asking anything else first - a role, a site
        // list - hides that, which is why nothing is asked here before the check.
        $access->checkUserHasCapability($idSite, TestCustomCap::ID);
    }

    public function testGetCapabilitiesForSiteLoadsSitesWhenNothingElseHasLoadedThemYet()
    {
        Piwik::addAction('Access.Capability.addCapabilities', function (&$cap) {
            $cap[] = new TestCustomCap();
        });
        ...
        $this->assertContains(TestCustomCap::ID, $access->getCapabilitiesForSite($idSite));
    }
    ...
    public function testGetRoleForSiteDoesNotEnumerateSitesForCappedSuperuserToken()
    {
        ...
        // Enumerating every site to answer a single-site question would make tracker authentication scan
        // the whole site table per request, and turn a transient failure of that query into a silent
        // rejection of a token that should have been accepted.
    ...
    public function testGetRawSitesWithSomeViewAccessWithinTokenScopeReturnsNoRowsForTokenWithoutResolvableScope()
    {
        $authMock = $this->createPiwikAuthMockInstance();
        $authMock->expects($this->once())
            ->method('authenticate')
        ...
        $access->expects($this->any())
            ->method('getRawSitesWithSomeViewAccess')
            ->will($this->returnValue([
                ['idsite' => 1, 'access' => 'admin'],
                ['idsite' => 2, 'access' => 'view'],
            ]));

        $this->assertTrue($access->reloadAccess($authMock));

        // The loaded lists answer with nothing for this token, and the access rows a site-listing API
        // reads have to agree with them - otherwise the sites come back complete, names and URLs included.
        $this->assertEmpty($access->getSitesIdWithAtLeastViewAccess());
        $this->assertSame([], $access->getRawSitesWithSomeViewAccessWithinTokenScope('login'));
    }

    public function testGetRawSitesWithSomeViewAccessWithinTokenScopeCapsRolesToTheTokenAccessLevel()
    {
        $authMock = $this->createPiwikAuthMockInstance();
        $authMock->expects($this->once())
            ->method('authenticate')
        ...
        $access->expects($this->any())
            ...
            ->will($this->returnValue([
                ['idsite' => 1, 'access' => 'admin'],
                ['idsite' => 2, 'access' => 'view'],
                ['idsite' => 3, 'access' => 'notarole'],
            ]));

        $this->assertTrue($access->reloadAccess($authMock));

        // The site stays reachable at the capped role, and the row this version cannot place on the scale
        // at all is dropped rather than passed on uncapped.
        $this->assertSame(
            [
                ['idsite' => 1, 'access' => 'view'],
                ['idsite' => 2, 'access' => 'view'],
            ],
            $access->getRawSitesWithSomeViewAccessWithinTokenScope('login')
        );
    }

    public function testGetRawSitesWithSomeViewAccessWithinTokenScopeLeavesRowsOfAnotherLoginUntouched()
    {
        $authMock = $this->createPiwikAuthMockInstance();
        $authMock->expects($this->once())
            ->method('authenticate')
        ...
        $access->expects($this->any())
            ->method('getRawSitesWithSomeViewAccess')
            ->will($this->returnValue([['idsite' => 1, 'access' => 'admin']]));

        $this->assertTrue($access->reloadAccess($authMock));

        // The cap describes what this credential may see of its own user. Another login's access is a
        // different question - the one a scheduled task asks, which carries no token to be capped by.
        $this->assertSame(
            [['idsite' => 1, 'access' => 'admin']],
            $access->getRawSitesWithSomeViewAccessWithinTokenScope('otherlogin')
        );
    }
