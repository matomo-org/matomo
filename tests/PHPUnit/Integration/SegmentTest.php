                  ...
                  (
                    ...
                    WHERE 
                      (
                        ...
                        OR log_link_visit_action.idaction_content_name = ?
                      ) 
                      AND(
        ...
        $expected = array(
            "sql"  => "
                ...
                FROM (
                    ...
                    LEFT JOIN $logAction AS log_action_segment_log_link_visit_actionidaction_url ON log_link_visit_action.idaction_url = log_action_segment_log_link_visit_actionidaction_url.idaction
                    WHERE (
                            HOUR(log_visit.visit_last_action_time) = ?
                            OR (1 = 0) " . // pageUrl==xyz
                        " )
                        AND (
                            (
...
SQL;
    ...
    public function test_segmentWithFilterMatchGeneratesCorrectSql()
    {
        $this->addSegmentWithSqlFilterMatch();
        ...
        $segment = 'pageUrl2=@abc,pageUrl2==def,pageUrl2!@ghi;pageUrl2!=ghi';
        $segment = new Segment($segment, $idSites = array());
        ...
        $expected = [
            'sql' => "SELECT sum(log_inner.idvisit)
                    FROM (
                        SELECT log_visit.idvisit
                        FROM $logVisitTable AS log_visit
                        LEFT JOIN $logLinkVisitAction AS log_link_visit_action ON log_link_visit_action.idvisit = log_visit.idvisit
                                ...
                                (
                                    ...
                                    OR (
                                        (
                                            ...
                                            OR log_action_segment_log_link_visit_actionidaction_url.name NOT LIKE ?
                                            )
                                        AND log_action_segment_log_link_visit_actionidaction_url.type = '1'
                                        )
                                    )
    ...
    private function addSegmentWithSqlFilterMatch()
    {
        $pageUrl2 = new class() extends ActionDimension {
            protected $columnName = 'idaction_url';
            protected $segmentName = 'pageUrl2';
            ...
            public function __construct()
            {
                $this->sqlFilterMatch = function ($field, $matchType, $value, $join) {
                    if ($matchType == Segment\SegmentExpression::MATCH_EQUAL
                        || $matchType == Segment\SegmentExpression::MATCH_NOT_EQUAL
                    ) {
                        $table = $join['tableAlias'] ?? $join['table'];
                        $sql = "($table.name = ? AND $table.hash = CRC32(?))";
                        $bind = [$value, $value];
        ...
        Piwik::addAction('Dimension.addDimensions', function (&$dimensions) use ($pageUrl2) {
            $dimensions[] = $pageUrl2;
        });
    }
