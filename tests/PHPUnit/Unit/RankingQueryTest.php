        ...
        $expected = "
            SELECT
                CASE
                    ...
                    WHEN counterRollup > 0 THEN withCounter.`label`
                    WHEN counter = 11 AND counterRollup = 0 THEN withCounter.`label`
                    WHEN counter = 11 THEN 'Others'
                    ELSE withCounter.`label`
                END AS `label`,
                CASE
                    WHEN withCounter.`url` IS NULL THEN NULL
                    WHEN counter = 11 AND counterRollup = 0 THEN 'Others'
                    ELSE withCounter.`url`
                END AS `url`,
                `column`,
                sum(`columnSum`) AS `columnSum`
            FROM (
