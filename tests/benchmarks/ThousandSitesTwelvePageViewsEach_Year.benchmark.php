...
class Benchmarks_ThousandSitesTwelvePageViewsEach_Year extends Test_Integration
{
	...
	public function setUp()
	{
		...
		for ($i = 0; $i != 12; ++$i)
		{
			$visitTimes[] = $date->addMonth($i)->toString();
		}
		
		// add 12 page views per site (1 visit w/ 1 page view for 1 visitor per month of the year)
		foreach ($this->idSites as $idSite)
		{
	...
	{
		Piwik_VisitsSummary_API::get('all', 'year', $this->dateTime);
	}
	
	public function test_archiveGoals_andQueryAll()
	{
		Piwik_Goals_API::get('all', 'year', $this->dateTime);
	}
}
